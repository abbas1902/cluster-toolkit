...
output "instructions" {
  ...
  value       = <<-EOT
    ...
    you can use the following command to see the logs the cluster and the hostname:
      gcloud read 'labels.cluster_name=${local.slurm_cluster_name}'
      gcloud read 'labels.hostname=${local.slurm_cluster_name}-controller'
  EOT
}
